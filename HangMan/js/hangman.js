...
Vue.component('letter-button', {
    ...
    template:
        '<button class=\'keyboard-row-letter\' :id=\'letter\' :disabled=\'disabled\' @click=\'clicked()\'>{{ letter }}</button>',
    data: function () {
        return {
            disabled: false
        };
...
let app = new Vue({
    ...
    data: {
        ...
        // will bet set to the canvas 2d context
        ctx: ''
    },
    ...
    methods: {
        ...
        drawGallows: function (ctx) {
            ...
            ctx.fillStyle = '#f4d701';
            ctx.strokeStyle = '#f4d701';
            ctx.beginPath();
        ...
        updateCanvas: function (ctx) {
            ...
            // draw the right arm
                ...
                ctx.moveTo(
                    this.canvas.width * 0.4,
                    this.canvas.height / 5 + 55
                );
                ctx.lineTo(
                    this.canvas.width * 0.4 + 35,
        ...
        check: function (letter) {
            if (!this.gameOver) {
                ...
                if (
                    !this.wordDivs.some(function (value) {
                        return value == '';
                    })
                ) {
                    this.gameOver = true;
        ...
        // resets the game to the one-player mode and chooses new word
        onePlayer: function () {
            if (this.twoPlayers) {
                this.twoPlayers = false;
                this.currentWord =
                    this.words[randomInteger(0, this.words.length - 1)];
                this.restart();
            }
        },
        ...
        twoPlayer: function () {
            if (!this.twoPlayers) {
                ...
                while (!letters.test(this.currentWord)) {
                    try {
                        this.currentWord = prompt(
                            'Only letters please! Enter a word:'
                        ).toUpperCase();
                    } catch (e) {
                        this.onePlayer();
                        return;
